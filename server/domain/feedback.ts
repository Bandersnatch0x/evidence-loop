...
export class LocalFeedbackGenerator implements FeedbackGenerator {
  public generate(context: FeedbackContext): Promise<FeedbackOutput> {
    ...
    const delta =
      context.previousScore === undefined
        ...
        : context.score > context.previousScore
          ? `，比上一轮提高 ${context.score - context.previousScore} 分`
          : context.score === context.previousScore
            ? '，与上一轮持平'
            : `，比上一轮下降 ${context.previousScore - context.score} 分`

    if (failedCount === 0) {
      return Promise.resolve({
        summary: `全部可验证证据通过，当前得分 ${context.score} 分${delta}。本轮已完成任务闭环，可进入下一项能力训练。`,
        source: 'local-policy'
      })
    }
    ...
    return Promise.resolve({
      summary: `当前得分 ${context.score} 分${delta}，${String(failedCount)} 项证据未通过。优先处理“${focus}”，完成后重新提交验证。`,
      source: 'local-policy'
    })
  }
}
...
export class OpenAICompatibleFeedbackGenerator implements FeedbackGenerator {
  ...
  public async generate(context: FeedbackContext): Promise<FeedbackOutput> {
    try {
      ...
      if (!jsonMatch) throw new Error('LLM response was not JSON')
