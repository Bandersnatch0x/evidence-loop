# [wayfinder:map] EvidenceRing 产品化：学生自主刷题 + AI 辅导 + 教师学情

## Destination

把 EvidenceRing 从"证据评分 Demo"演进为**功能完整的产品原型**：学生真登录后自主刷题并获得三层 AI 辅导（讲解/对话/苏格拉底引导），老师能导入题目（含扫描 OCR）、导入学生名单分配账号、查看班级学情、并按薄弱点一键布置；学情自动闭环成下一批题。数据合规守 Demo 级（测试名单），AI 真正通电（接 LLM）。
