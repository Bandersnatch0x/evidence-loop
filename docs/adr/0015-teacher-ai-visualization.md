...
1. **几何以 `kind` 多态扩展。** MVP 先落地球棍类（`kind:'ball_stick'`，atoms/bonds），覆盖分子/晶体。Phase 4 兑现扩展点：`kind:'curve'`（预采样 3D 折线 + 可选 `secondaryPoints` 第二链），覆盖磁场螺旋、DNA 双链骨架等。电路/细胞等图元类仍走后续 `primitives`，不塞进球棍或曲线。
...
8. **通用 scene + 懒加载。** BallStickScene / CurveScene 均接收 Visualization 分支 props（几何来自 props 而非硬编码）。VisualizationGenerator 与 Visualizer 均 lazy import 对应 scene——避免把 three 拉进主 chunk（修复实测：主 index chunk 曾涨到 1495KB，lazy 后回落 ~572KB）。
...
- 球棍 + 曲线覆盖分子/晶体/磁场螺旋/DNA 骨架等常见演示；`kind` 多态仍为图元类留扩展点
